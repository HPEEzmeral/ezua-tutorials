...
libraries_to_install = ["fuzzywuzzy", "pycountry"]
...
def update_database_table(database_type, database, table):
    ...
    for row in rows:
        ...
        if curr_code == "EUR":
            ...
            if currency_rate is not None:
                ...
                unit_euro = unit_price / currency_rate

                # Calculate the total sales and round to two decimal places
                total_sales = unit_euro * quantity

                updated_rows.append((row[0], row[1], row[2], unit_euro, row[4], quantity, total_sales, currency_code, row[8], country_name, row[10]))
    ...
    query = f"INSERT INTO {table} (product_id, product, type, unit_price, unit, qty, total_sales, currency, store, country, year) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"

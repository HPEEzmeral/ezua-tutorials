...
def llm_service(
    ...
    top_p,
    context_check,
    llm_server_subdomain,
    model_name,
    request: gr.Request,
):
    SVC = f"{llm_server_subdomain}.{NAMESPACE}.{DOMAIN_NAME}"
...
def main(llm_server_subdomain, model_name):
    with gr.Blocks(theme=EzmeralTheme()) as app:
        ...
        clear_btn.click(
            lambda: [None, 0.2, 1, 100, 40, 0.4, True, None],
            [],
            outputs=[
                question,
                temperature,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-name", type=str, default="ensemble")
    args = parser.parse_args()

    logger.info("LLM Server Subdomain: %s", args.llm_server_subdomain)
    logger.info("Model Name: %s", args.model_name)

    main(args.llm_server_subdomain, args.model_name)

...
# Define the op_kwargs dictionary
op_kwargs = {
    'bucket_name': 'ezaf-demo',
    'file_name': 'Germany_sales_data_2019_2023.csv',
...
def run_clean_data_script(db_host=None, db_port=None, db_user=None, db_password=None, db_name=None, table_name=None):
    ...
    script_path = "/usr/local/airflow/dags/gitdags/Scripts/clean_data.py"  # Replace with the actual path to clean_data.py
    command = [
        "python",
        script_path,
        "-db", "mysql",
...
with DAG('load_s3_csv_to_mysql_db_germany', default_args=default_args, schedule_interval='0 7 * * *') as dag:
